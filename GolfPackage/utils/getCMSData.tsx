...
export class GetCMSData extends PureComponent<IGetCMSDataProps, IGetCMSDataState> {
    render() {
        ...
        return (
            <CmsContainer name={name} key={`get_${name}_cms_golf_package`}>
                <CmsSpot name='' spec={{ content_type }} render={({ item }) => {
                    const packageName = item.elements?.name?.value;
                    ...
                    const hasTeeTimes = item.elements?.course?.linkedItems.length > 0 || item.elements?.mandatory_courses?.linkedItems.length > 0;
